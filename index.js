...
const config = {
    ...
    nomer: 'wa.me/5511973027044', //MUDA AKI
...
client.on('group-participants-update', async (anu) => {
	...
	client.on('message-new', async (mek) => {
		try {
			...
			const ownerNumber = ["5511973027044@s.whatsapp.net"] //MUDA AKI
...
case 'update':
				case 'atualizacao':
				case 'nota':
					client.sendMessage(from, update(prefix), text)
					break
case 'anime':
                                        ...
                                        client.sendMessage(from, buffer, image, {quoted: mek, caption: 'akii brooo'})
                                        break
case 'hentai':
                                        gatauda = body.slice(6)
                                        reply(mess.wait)
					...
					if ((isMedia && !mek.message.videoMessage || isQuotedImage) && args.length == 0) {
						...
						await removeBackgroundFromImageFile({path: media, apiKey: keyrmbg.result, size: 'auto', type: 'auto', ranp}).then(res => {
							...
							fs.writeFileSync(ranp, buffer, (err) => {
								if (err) return reply('Falha, tente novamente mais tarde.')
							})
